...
namespace StockTrading.Infrastructure.ExternalServices.KoreaInvestment.Common.Helpers;
...
public static class KisValidationHelper
{
    ...
    {
        if (!DateTime.TryParseExact(dateString, ParsingConfig.ValidationConstants.Date.Format, null,
    ...
    {
        var start = DateTime.ParseExact(startDate, ParsingConfig.ValidationConstants.Date.Format, null);
        var end = DateTime.ParseExact(endDate, ParsingConfig.ValidationConstants.Date.Format, null);

        if ((end - start).TotalDays > ParsingConfig.ValidationConstants.Date.MaxInquiryDays)
